...
def add(x):
    if str(getStreetName(x).lower().strip()) in (street_details.keys()):
        sys.stdout.write ('Error: Add Failure. Duplicate street found.'+'\n')
        sys.stdout.flush()
    else:
        getRoadDetails(x)
...
def change(x):
    if str(getStreetName(x).lower().strip()) in (street_details.keys()):
        ...
        sys.stdout.write('Error: Cannot update item. Street not found.'+'\n')
        sys.stdout.flush()
...
def remove(x):
    if str(getStreetName(x).lower().strip()) in (street_details.keys()):
        ...
        sys.stdout.write ('Error: Cannot remove item. Street not found.'+'\n')
        sys.stdout.flush()
...
def vertices(x):
    ...
    #sys.stdout.write ('V '+str(len(vertices_dictionary))+'\n')
    print "V", len(vertices_dictionary)+1
    sys.stdout.flush()
    #print ('V '+str(count-1))
...
def setofEdges(x):
    ...
    for items in E:
        if items == E[-1]:
            ...
            new = new + str(items) + ","
    print 'E {' + new.replace('(', '<').replace(')', '>').replace(' ', '') + '}'
    sys.stdout.flush()

    # newlist = list(E)
    # print 'E {' + newlist.replace('(', '<').replace(')', '>').replace(' ', '') + '}'
    # sys.stdout.flush()
    #
    # sys.stdout.write ('E {')
    # sys.stdout.flush()
    ...
    #     # print (newlist[i]),
    #     if i == newlist[-1]:
    #         #print (i.__str__().replace('(', '<').replace(')', '>').replace(' ', ''))
    #         #print (newlist[i])
